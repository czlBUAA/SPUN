...
def euler2quatt(pitch, yaw, roll):
    ...
    q = np.matrix([[-sin_yaw*cos_roll*cos_pitch - cos_yaw*sin_roll*sin_pitch],
                   [-cos_yaw*sin_roll*cos_pitch + sin_yaw*cos_roll*sin_pitch],
                   [sin_yaw*sin_roll*cos_pitch - cos_yaw*cos_roll*sin_pitch],
                   [cos_yaw*cos_roll*cos_pitch + sin_yaw*sin_roll*sin_pitch]])
...
def quat2SO3(q):
    ...
    return R
def quat2SO33(q):
    """ Convert right-handed quaternion to SO3 according to
        Trawny and Roumeliotis "Indirect kalman filter for 3d attitude estimation" (valid)"""
...
def quat_weighted_avg(Q,W):
    ...
    q_avg = v[:, idx[-1]] # 0
    #对矩阵M进行求特征值和对应的特征向量，最大特征值对应的特征向量，就是最优
