...
import utils
import symmartix
class BGFixedDispersionLoss(object):
    """
    ...
    def _bingham_loss_fixed_dispersion_single_sample(target_qr, target_qt, bd_m, bd_z, G, ga_cov):
        ...
        target_qt = torch.cat((target_qt, torch.zeros(1,  1, device=target_qt.get_device(), dtype=target_qt.dtype)), dim=1)
        target_qd = 0.5*utils.Qmultiply(target_qt[0], target_qr[0])
