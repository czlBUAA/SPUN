...
class mutimodal(object):
    def __init__(self, mixture_component_count=3,
                 fixed_dispersion_stage=80,
                 fixed_param_z=[-1, -1, -1, 0], fixed_param_cov=[-1,-1,-1,-1]):
        self._num_components = mixture_component_count
        self._fixed_dispersion_stage = fixed_dispersion_stage
        self._softmax = torch.nn.Softmax(dim=1)
        print("initial")
    def __call__(self, target_qr,  target_qt, output, epoch):
        ...
        for i in range(batch_size):
            ...
            for j in range(self._num_components):
                B, G, cov = self._output_to_B_G(output[i, (j*37+1):((j+1)*37)].unsqueeze(0))
                bd_likelihood = torch.sum(self._BG_loss(target_qr[i].unsqueeze(0), target_qt[i].unsqueeze(0), B, G, cov))
                current_likelihood += weights[i, j] * \
                    bd_likelihood.squeeze()
            log_likelihood += torch.log(current_likelihood)
            
        loss = -log_likelihood
        log_likelihood /= batch_size
        return loss, log_likelihood
    def statistics(self, target_qr,  target_qt, output, epoch):
        ...
        stats = {
            ...
            "gt_loc": target_qt.detach().cpu().numpy()
        }
        return stats
    @staticmethod
    def _BG_loss(target_qr, target_qt, B, G, cov):
        ...
        likelihoods = torch.exp((torch.bmm(torch.bmm(
                ...
                cov),
                (target_qd-torch.bmm(G,target_qr.unsqueeze(2)).squeeze()).unsqueeze(2))).squeeze()) / ((norm_const) * (2 * math.pi* torch.sqrt(torch.linalg.det( -0.5 *torch.linalg.inv(cov)))))
        return likelihoods
